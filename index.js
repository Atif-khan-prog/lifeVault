...
app.put('/mainPage/finance/:username/edit/:id', (req, res) => {
    ...
    const { description, amount, category, date } = req.body;
    const query = 'UPDATE finance SET description = ?, amount = ?, category = ?, date = ?, created_at = NOW() WHERE id = ?';

    database.query(query, [description, amount, category, date, id], (err) => {
        if (err) return res.status(500).send('Error updating: ' + err.message);
        res.redirect(`/mainPage/finance/${username}`);
    });
});
...
app.delete('/mainPage/finance/:username/delete/:id', (req, res) => {
    ...
    database.query(query, [id], (err) => {
        if (err) return res.send('Error deleting: ' + err.message);
        res.redirect(`/mainPage/finance/${username}`);
    });
});
